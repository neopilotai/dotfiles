...
gh_clone_all.sh neopilotai ~/Work
pull_all.sh ~/Work/neopilotai
...
find ~/Work/neopilotai -depth 2 -name package.json -exec dirname {} \; | while read dir; do
...
# install husky git hooks in neopilotai/mono
if [ -d ~/Work/neopilotai/mono ]; then
  echo "Installing husky git hooks in ~/Work/neopilotai/mono"
  (cd ~/Work/neopilotai/mono && husky)
fi
